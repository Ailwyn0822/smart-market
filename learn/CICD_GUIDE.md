...
**Sync i18n (`.github/workflows/sync-i18n.yml`)：**

- **手動觸發**（`workflow_dispatch`），不影響 CI/CD 主流程
- 從 Google Sheets 讀取翻譯資料，更新 `frontend-client/i18n/language/` 目錄
- 自動 commit 並 push（若有異動）
- 需要在 GitHub Secrets 設定 `GOOGLE_CREDENTIALS_JSON`（Service Account JSON）
...
| `/api/notifications/stream` | `sm_backend:8080`            | SSE 長連線通知（**必須在 `/api/` 之前**，已設定 `proxy_buffering off`） |
...
> ⚠️ `/api/_nuxt_icon/` 和 `/api/notifications/stream` 都必須定義在 `/api/` 前面，否則會被錯誤導向 NestJS backend 造成 404。
...
| `.github/workflows/sync-i18n.yml` | 手動觸發：從 Google Sheets 同步 i18n 翻譯資料 |
